...
public class RobotImpl extends Robot {
	...
	public void moveForward() throws InvalidMovementException {
		...
		case NORTH:
			newPosition = new Position(position.getX(), position.getY() + STEP_SIZE, Position.Orientation.NORTH);
			break;
		case EAST:
			newPosition = new Position(position.getX() + STEP_SIZE, position.getY(), Position.Orientation.EAST);
			break;
		case SOUTH:
			newPosition = new Position(position.getX(), position.getY() - STEP_SIZE, Position.Orientation.SOUTH);
			break;
		case WEST:
			newPosition = new Position(position.getX() - STEP_SIZE, position.getY(), Position.Orientation.WEST);
			break;
		}
		place(newPosition);
	}
	...
	public void turnRight() {
		...
		position.setOrientation(Position.Orientation.getInstance((position.getOrientation().getDegree() + TURNING_DEGREE) % CIRCLE));
	}

	@Override
	public void turnLeft() {
		// add 360 to offset the negative value because of counter clockwise rotation
		position.setOrientation(Position.Orientation.getInstance((position.getOrientation().getDegree() - TURNING_DEGREE + CIRCLE) % CIRCLE));
	}
